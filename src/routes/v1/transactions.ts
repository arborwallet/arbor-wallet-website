...
import { addressInfo, sanitizeHex, toAddress, toCoinId } from 'chia-rpc';
        ...
        for (const record of recordsResult.coin_records) {
            ...
            if (parentResult.coin_record.coin.puzzle_hash !== puzzleHash) {
                ...
                group.transactions.push({
                    sender: toAddress(
                        fromHex(
                            sanitizeHex(
                                parentResult.coin_record.coin.puzzle_hash
                            )
                        ),
                        address.prefix
                    ),
            ...
            if (record.spent) {
                ...
                for (const child of updatesResult.additions.filter(
                    (record) => record.coin.parent_coin_info === toHex(coinId)
                )) {
                    if (child.coin.puzzle_hash !== puzzleHash)
                        group.transactions.push({
                            destination: toAddress(
                                fromHex(sanitizeHex(child.coin.puzzle_hash)),
                                address.prefix
                            ),
